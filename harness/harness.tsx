...
window.renderMermaid = async (source, opts) => {
  ...
  await createMermaidDiagram(editor, source, {
    async onUnsupportedDiagram(svgString: string) {
      ...
      // (pie, gantt, class, ER, ...). Still a real diagram, just not the native
      // tldraw aesthetic.
  ...
  // Export the same layout twice — transparent background, tldraw's light then
  // dark theme colors — so the page can swap variants per color scheme.
